...
public class IfElseRequirement<T> implements Requirement<T> {
    ...
    public boolean test(T type, ConfigurationSection config) {
        ...
        Optional<? extends Requirement<T>> leftRequirement = ActionAPI.createRequirement(
                getIdentifier(),
                left.getString("type"),
                left,
                (Class<T>) type.getClass());
        Optional<? extends Requirement<T>> rightRequirement = ActionAPI.createRequirement(
                getIdentifier(),
                right.getString("type"),
                right,
                (Class<T>) type.getClass());
